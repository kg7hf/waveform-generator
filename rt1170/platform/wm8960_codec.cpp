...
// WM8960 SAI and DMA transport
// Guide: docs/modules/rt1170-player-and-audio.md
// Ownership: ISR publishes completed DMA work; audio task invokes the client hook.
// Contract: Hook must not allocate, log or perform I/O; lock-free counters report timing/errors.

// Generator-local import; see wm8960_codec.hpp for hardware validation limits.
...
{
    return configured_ ? Result<std::uint8_t> {receive_level_percent_} :
           Result<std::uint8_t> {Status{StatusCode::unavailable, "WM8960 codec not configured"}};
}

Result<std::uint8_t> Wm8960Codec::transmit_level_percent() const noexcept
{
    return configured_ ? Result<std::uint8_t> {transmit_level_percent_} :
           Result<std::uint8_t> {Status{StatusCode::unavailable, "WM8960 codec not configured"}};
}
...
{
    rx_produced_ = rx_produced_ + 1U; // Sequentially consistent publication to the audio task.
...
{
    rx_errors_ = rx_errors_ + 1U; // Sequentially consistent publication to the audio task.
}

void Wm8960Codec::on_tx_error() noexcept
{
    tx_errors_ = tx_errors_ + 1U; // Sequentially consistent publication to the audio task.
}
