...
// Guide: docs/modules/protocols-media-and-artifacts.md
// Ownership: One worker owns local FatFs generation and pending response state.
// Contract: Honor media ownership and cancellation; terminal publication follows completed writes.
...
std::atomic<bool> request_pending{}, response_pending{}, disconnected{};
static_assert(std::atomic<bool>::is_always_lock_free);
std::atomic<bool> processing_request{};
...
class FileSink final : public waveform_source::ByteSink
{
    bool write(const std::uint8_t* bytes, std::size_t count) noexcept override
    {
        if (!output_open)
        {
            if (f_open(&output_file, temporary_path, FA_WRITE | FA_CREATE_NEW) != FR_OK)
            {
                return false;
            }
...
void close_files() noexcept
{
    if (payload_open)
    {
        (void)f_close(&payload_file);
        payload_open = false;
    }

    if (output_open)
    {
        (void)f_close(&output_file);
        output_open = false;
    }

    if (mounted)
    {
        (void)f_mount(nullptr, "2:/", 0);
        mounted = false;
    }
}
...
bool mount() noexcept
{
    ...
    if (directory != FR_OK && directory != FR_EXIST)
    {
        close_files();
        return false;
    }
...
void make_path(char* path, const char* filename, const char* extension = nullptr) noexcept
{
    ...
    if (extension)
    {
        std::strcpy(std::strrchr(path, '.'), extension);
    }
}
...
const char* start_generation() noexcept
{
    ...
    if (!mount())
    {
        return "MEDIA_NOT_LOCAL";
    }

    if (!absent(output_path) || !absent(manifest_path) || !absent(temporary_path) || !absent(manifest_temporary_path))
    {
    ...
    if (f_open(&payload_file, input_path, FA_READ) != FR_OK)
    {
        return "BAD_PAYLOAD_FILE";
    }

    payload_open = true;
    const auto bytes = f_size(&payload_file);

    if (bytes > tx_protocol::maximum_upload_bytes)
    {
    ...
    if (!encoder)
    {
        return "ENCODER_WORKSPACE_ERROR";
    }
    ...
    {
        std::snprintf(selected_profile, sizeof(selected_profile), "%u:%s", static_cast<unsigned>(selected_rate), interleave);
    }
...
bool publish_artifact() noexcept
{
    ...
    if (!output_closed || !payload_closed)
    {
        return false;
    }
    ...
    const int count = std::snprintf(metadata, sizeof(metadata),
                                    ...
                                    "\"build_id\":\"%s\",\"profile\":\"%s\",\"sample_rate_hz\":48000,"
                                    ...
                                    descriptor->id, descriptor->version, WFG_BUILD_ID, selected_profile,
    ...
    if (count <= 0 || static_cast<std::size_t>(count) >= sizeof(metadata))
    {
        return false;
    }

    if (f_open(&manifest_file, manifest_temporary_path, FA_WRITE | FA_CREATE_NEW) != FR_OK)
    {
        return false;
    }
    ...
    if (f_rename(temporary_path, output_path) != FR_OK)
    {
        return false;
    }
...
bool submit_tx_request(const tx_protocol::Request& request) noexcept
{
    ...
    if (request_pending || response_pending || disconnected)
    {
        taskEXIT_CRITICAL();
        return false;
    }
...
void service_tx_artifact() noexcept
{
    ...
    if (request_pending)
    {
        ...
        else if (request.kind == Kind::reset)
        {
            ...
            selected_rate = 600;
            selected_interleave = 1;
            selected_encoder[0] = 0;
            selected_profile[0] = 0;
            select_output("GEN.WAV");
        }
        else if (player_live_snapshot().busy || snapshot.state == 2)
        {
            error = "BUSY";
        }
        else if (request.kind == Kind::mode)
        {
            if (!tx_protocol::mode_token(request.rate, request.interleave))
            {
                error = "BAD_MODE";
            }
            else
            {
                ...
                selected_interleave = request.interleave;
                selected_encoder[0] = 0;
                selected_profile[0] = 0;
            }
        }
        else if (request.kind == Kind::select)
        {
            ...
            else if (!tx_protocol::valid_filename(request.name, "WAV"))
            {
                error = "BAD_FILENAME";
            }
            else
        ...
        else if (request.kind == Kind::data)
        {
            ...
            {
                error = "SELECT_NEW_FILE";
            }
            else if (request.size == 0 || request.size > tx_protocol::maximum_payload ||
                     request.size > tx_protocol::maximum_upload_bytes - snapshot.payload_bytes)
            {
                error = "PAYLOAD_TOO_LARGE";
            }
            else if (!mount())
            {
                error = "MEDIA_NOT_LOCAL";
            }
            else
            {
                if (!payload_open)
                {
                    if (f_open(&payload_file, input_path, FA_WRITE | FA_CREATE_NEW) != FR_OK)
                    {
                        error = "PAYLOAD_EXISTS_OR_IO_ERROR";
                ...
                if (!error)
                {
                    ...
                    if (f_write(&payload_file, request.data, request.size, &actual) != FR_OK || actual != request.size)
                    {
                        error = "PAYLOAD_WRITE_ERROR";
                        transaction_error = true;
                    }
                    else
                    {
        ...
        else if (request.kind == Kind::generate_file)
        {
            ...
            else if (!tx_protocol::valid_filename(request.name, "WAV") || !tx_protocol::valid_filename(request.input_name, "BIN"))
            {
                error = "BAD_FILENAME";
            }
            else
    ...
    if (snapshot.state == 2)
    {
        ...
        taskENTER_CRITICAL();
        snapshot.frames_written = writer->frames_written();
        taskEXIT_CRITICAL();

        if (state == waveform_source::JobState::failed)
        {
            fail(writer->status().message);
        }
        else if (state == waveform_source::JobState::complete && !publish_artifact())
        {
            fail("ARTIFACT_FINALIZE_ERROR");
        }
    }
}
}
