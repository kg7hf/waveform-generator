...
void respond_info() noexcept
{
    char text[320] {};
    std::snprintf(text, sizeof(text),
                  "MODEM INFO:VERSION=0.2.0;BOARD=MIMXRT1170-EVK;KIND=waveform-generator;SERIAL=%08lX%08lX;PROTOCOL=DD008/1;MAX_UPLOAD=1048576;BUILD_ID=%s\n",
                  static_cast<unsigned long>(OCOTP->FUSEN[1].FUSE),
                  static_cast<unsigned long>(OCOTP->FUSEN[2].FUSE), WFG_BUILD_ID);
    respond(text);
}
