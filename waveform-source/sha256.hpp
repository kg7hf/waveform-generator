...
public:
    void update(const void* input, std::size_t count) noexcept
    {
        ...
        for (std::size_t i = 0U; i < count; ++i)
        {
            ...
            if (used_ == block_.size()) { compress(); used_ = 0U; }
        }
    }
    ...
    void hex(char (&output)[65]) const noexcept
    {
        ...
        while (copy.used_ != 56U) { copy.update(&zero, 1U); }
        std::uint8_t length[8]{};
        for (std::size_t i = 0U; i < 8U; ++i) { length[7U - i] = static_cast<std::uint8_t>(bits >> (8U * i)); }
        copy.update(length, sizeof(length));
        constexpr char digits[] = "0123456789abcdef";
        std::size_t position{};
        for (const auto word : copy.state_)
        {
            for (int shift = 28; shift >= 0; shift -= 4) { output[position++] = digits[(word >> shift) & 15U]; }
        }
        output[position] = '\0';
    }
