...
// WAV artifact job
// Guide: docs/modules/waveform-source-and-encoders.md
// Ownership: Borrows encoder, payload and sink until completion or cancellation.
// Contract: Writes bounded PCM chunks; invalid counts/non-finite samples fail before PCM publication.
...
{
    if (source_ != nullptr && state_ == JobState::running)
    {
        source_->stop();
    }
                                 ...
                                 WavEncoding encoding) noexcept
{
    ...
    if (encoding != WavEncoding::pcm24 && encoding != WavEncoding::pcm16_legacy)
    {
        (void)fail({m110::StatusCode::invalid_argument, "unsupported WAV encoding"});
        return status_;
    }
    ...
    put32(header.data() + 4U, 36U + data_bytes);
    std::memcpy(header.data() + 8U, "WAVEfmt ", 8U);
    put32(header.data() + 16U, 16U);
    put16(header.data() + 20U, 1U);
    put16(header.data() + 22U, 1U);
    put32(header.data() + 24U, 48000U);
    ...
    put32(header.data() + 40U, data_bytes);

    if (!sink_->write(header.data(), header.size()))
    {
        (void)fail({m110::StatusCode::io_error, "failed to write native WAV header"});
        return status_;
    }
...
std::size_t WavGenerator::read(std::uint8_t* bytes, std::size_t capacity) noexcept
{
    ...
    if (count > capacity || count > payload_bytes_ - payload_read_)
    {
        return 0U;
    }
...
{
    if (state_ != JobState::running || max_frames == 0U)
    {
        return state_;
    }
    ...
    if (frames_written_ < waveform_frames_)
    {
        ...
        if (!source_->status().is_ok())
        {
            return fail(source_->status());
        }

        if (count == 0U || count > request)
        {
            return fail({m110::StatusCode::io_error, "encoder did not honor its declared frame count"});
        }
    }
    ...
    if (count == 0U)
    {
        return fail({m110::StatusCode::io_error, "encoder ended without consuming its declared payload"});
    }

    for (std::size_t i = 0U; i < count; ++i)
    {
        if (!std::isfinite(frames_[i]))
        {
            return fail({m110::StatusCode::invalid_argument, "encoder returned a nonfinite sample"});
        }
    ...
    if (!sink_->write(bytes_.data(), output_bytes))
    {
        return fail({m110::StatusCode::io_error, "failed to write native WAV PCM"});
    }
    ...
    {
        if (payload_read_ != payload_bytes_)
        {
            return fail({m110::StatusCode::io_error, "native payload consumption was incomplete"});
        }
