...
// Waveform source interfaces
// Guide: docs/modules/waveform-source-and-encoders.md
// Ownership: Buffers and source/sink objects are caller-owned and must outlive each job.
// Contract: Return checked status and never report more samples than the supplied capacity.
...
public:
            ...
            std::size_t capacity) noexcept
    {
        ...
        if (count > request)
        {
            // A broken adapter must not turn its reported count into an
            // out-of-bounds read of the fixed legacy conversion buffer.
            return 0U;
        }
