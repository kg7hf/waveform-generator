...
| Workflow | Tools |
|---|---|
| Make clean audio | `wfg_m110_tx.exe`, `stage_wav.py make-tone` |
| Add impairments | `render_scenario.py`, `signal_lab_render.exe` |
| Build a campaign | `corpus_phase1.py`, `m110_score.py` |
...
```text
wfg_m110_tx RATE INTERLEAVE OUTPUT MESSAGE...
wfg_m110_tx RATE INTERLEAVE OUTPUT --file BINARY_PAYLOAD
```

Rates are `75|150|300|600|1200|2400|4800`. Interleave is
`short|long|zero`; the pair must be one of the 13 supported profiles. Output is
`.wav` or raw `.pcm`.
...
```powershell
build\host-release\host\tx\wfg_m110_tx.exe 600 long clean.wav "TEST"
...
python host/m110_score.py --decoder m110_app_decode.exe `
  --engine siso --summary score impaired.wav
```
