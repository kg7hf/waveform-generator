...
This guide covers the Phase 1 corpus specification, reference generation,
...
```powershell
python host/corpus_phase1.py --stage plan
```

Run the complete pipeline with explicit producer and decoder executables:

```powershell
python host/corpus_phase1.py `
  --spec corpus/phase1-corpus.json `
...
```powershell
python host/corpus_phase1.py --stage verify
```
...
|---|---|
| Matrix/spec | `corpus/phase1-corpus.json` |
| Orchestration | `host/corpus_phase1.py` |
