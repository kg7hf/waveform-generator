...
This directory is the stable architecture layer for waveform-generator. Phase
reports answer "what happened during a phase?" These module guides answer "how
does this part work now, how do I use it, and what must remain true when I change
it?"
...
| [Native M110B Waveform](native-m110-waveform.md) | M110B framing, FEC, interleave, symbols, streaming modulation, and supported profiles | Changing the built-in M110B producer |
...
| [Build Contracts and Provenance](build-contracts-and-provenance.md) | CMake targets, presets, source identity, dependency locks, imports, and boundary audits | Adding sources, targets, or dependencies |
...
| `native-m110/` | Native M110B Waveform |
...
| `host/corpus_phase1.py`, `host/render_scenario.py`, `host/m110_score.py`, `corpus/`, `test-vectors/` | Corpus Rendering and Scoring |
| `CMakeLists.txt`, `CMakePresets.json`, `cmake/`, `scripts/`, `schema/`, dependency and source ledgers | Build Contracts and Provenance |
...
4. Read Waveform Source and Encoders, then Native M110B Waveform, for source creation.
...
8. Read Build Contracts and Provenance before changing dependencies or target structure.

## Update rule

When behavior changes, update the owning module guide in the same change. A phase
report may record the decision and evidence, but it does not replace the stable
module explanation. Keep claims bounded: host equivalence, target build success,
