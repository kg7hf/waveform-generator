...
// Ownership: One fixed JSON workspace is guarded against concurrent parse calls.
// Contract: Failure returns a static diagnostic; never parse on an audio deadline.
...
std::atomic_flag scenario_parser_busy = ATOMIC_FLAG_INIT;
...
{
    if (scenario_parser_busy.test_and_set())
    {
        return set_error(error, "scenario parser is busy");
    }

    struct ParserRelease
    {
        ~ParserRelease() noexcept
        {
            scenario_parser_busy.clear();
        }
    } release;
    ...
    if (seed != nullptr)
    {
        if (seed->type != Type::number || !std::isfinite(seed->number) || seed->number < 0.0 ||
                seed->number >= 18446744073709551616.0 || std::trunc(seed->number) != seed->number)
        {
    ...
    {
        return validate_scenario(scenario, error);
    }
