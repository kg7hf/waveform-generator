...
{
public:
    explicit SampleSlipImpairment(const SampleSlipParams& params) noexcept : params_
    {
        params
    } {}
...
{
    if (storage == nullptr || reinterpret_cast<std::uintptr_t>(storage) % impairment_alignment != 0U || storage_bytes < sample_slip_size())
    {
        return nullptr;
    }
