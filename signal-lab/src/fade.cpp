...
{
public:
    explicit FadeImpairment(const FadeParams& params) noexcept : params_
    {
        params
    } {}
...
{
    if (storage == nullptr || reinterpret_cast<std::uintptr_t>(storage) % impairment_alignment != 0U || storage_bytes < fade_size())
    {
        return nullptr;
    }
