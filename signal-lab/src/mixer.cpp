...
// Ownership: Borrows an immutable scenario; owns the placement arena and scratch.
// Contract: configure validates geometry; process latches errors and callers discard rejected output.
...
namespace
{
constexpr Scenario passthrough_scenario = [] { Scenario result{}; result.source_gain_db = 0.0; return result; }();
} // namespace
...
void Engine::destroy_stages() noexcept
{
    for (std::uint32_t index = 0U; index < stage_count_; ++index)
    {
        stages_[index]->~Impairment();
...
bool Engine::configure(const Scenario& scenario, double reference_rms, std::uint64_t total_frames, const char** error) noexcept
{
    ...
    if (!(reference_rms > 0.0) || !std::isfinite(reference_rms))
    {
        if (error != nullptr)
        {
    ...
    if (!std::isfinite(reference_rms * gain))
    {
        if (error)
        {
            *error = "scaled reference RMS is not finite";
        }
...
std::size_t Engine::process(const std::int16_t* input, std::size_t frames, std::int16_t* output, std::size_t output_capacity) noexcept
{
    ...
    for (const float value : std::span{work_, count})
    {
        if (!std::isfinite(value))
        {
            process_error_ = ProcessError::nonfinite_sample;
            return 0U;
        }
    }
                            ...
                            std::size_t output_capacity) noexcept
{
    ...
    for (const float value : std::span{input, frames})
    {
        if (!std::isfinite(value))
        {
            process_error_ = ProcessError::nonfinite_sample;
