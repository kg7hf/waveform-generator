...
public:
    ...
    // Owns placement-constructed stages whose scratch pointers refer into this
    // instance. Moving/copying it would leave pointers into the old arena.
    ...
    // stage/clip-rejected block can consume input and overwrite the output buffer;
    // callers must discard that buffer. No output statistics/digest are committed.
    // Non-finite input is rejected before input state changes.
    ...
    [[nodiscard]] std::size_t process(std::span<const float> input, std::span<float> output) noexcept
    {
        return process(input.data(), input.size(), output.data(), output.size());
    }

    [[nodiscard]] std::size_t process(std::span<const std::int16_t> input, std::span<std::int16_t> output) noexcept
    {
        return process(input.data(), input.size(), output.data(), output.size());
    }
