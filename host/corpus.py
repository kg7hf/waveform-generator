...
"""Corpus orchestrator: references -> validate -> render impairments -> score -> summary.

    python host/corpus.py --spec corpus/default-corpus.json \
...
def stage_summary(spec, root, jobs_list, scores, engines):
    ...
    with open(manifests / "corpus-manifest.json", "w", encoding="utf-8", newline="\n") as handle:
        json.dump(manifest, handle, indent=1, sort_keys=True, allow_nan=False)
        handle.write("\n")
    tables = {}
    for engine in engines:
        rows = m110_score.write_summary(scores[engine], manifests / ("scores-%s.csv" % engine), manifests / ("scores-%s.json" % engine))
    ...
    with open(manifests / "summary.json", "w", encoding="utf-8", newline="\n") as handle:
...
def main(argv=None):
    ...
    parser.add_argument("--spec", type=Path, default=Path(__file__).resolve().parent.parent / "corpus" / "default-corpus.json")
    ...
    try:
        ...
        print("corpus: %s" % error, file=sys.stderr)
        return 2
    return 0
