...
"""Scenario -> impaired WAV + JSON sidecar (Python renderer)."""
...
from .wav_io import Pcm24Writer, pcm_region_sha256, quantize_pcm24, read_float, require_canonical, sha256_file, wav_info
...
def load_source_sidecar(source_path):
    ...
    try:
        ...
        return None
    keep = {key: data[key] for key in ("schema", "test_id", "mode", "rate_bps", "interleave", "payload", "framing", "wav", "generator", "encoder", "profile", "build_id") if key in data}
...
def render(scenario, output_wav, sidecar_path=None, base_dir=None, overwrite=False, block=BLOCK, created_utc=None):
    ...
    require_canonical(info, allow_legacy_pcm16=True)
    total = len(samples)
    ...
    first_clip = None
    try:
        with Pcm24Writer(temp) as writer:
            for out in pipeline.run(samples * gain, block=block):
                pcm, block_clipped = quantize_pcm24(out)
                if block_clipped and scenario["clip_policy"] == "reject":
                    scaled = out * 8388608.0
                    index = int(np.flatnonzero((scaled < -8388608.0) | (scaled > 8388607.0))[0])
                    raise RenderError("clipping at output frame %d and clip_policy=reject" % (frames + index))
                if block_clipped and first_clip is None:
                    scaled = out * 8388608.0
                    first_clip = frames + int(np.flatnonzero((scaled < -8388608.0) | (scaled > 8388607.0))[0])
                clipped += block_clipped
                emitted = pcm.astype(np.float64) / 8388608.0
    ...
    sidecar = {
        ...
        "output": dict(wav_artifact(output_wav), clipped_samples=clipped, first_clipped_frame=first_clip,
                       ...
                       sample_rate_hz=FS, pcm_format="pcm_s24le"),
        "conventions": {
            "order": "source * source_gain -> impairments in list order -> PCM24 round-half-even, saturate",
