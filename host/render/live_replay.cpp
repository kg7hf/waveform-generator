...
public:
    ...
    bool array(std::vector<std::string_view>& values)
    {
        ...
        for (;;)
        {
            ...
            if (values.size() > signal_lab::live_capture_capacity)
            {
                return false;
            }
    ...
    bool decoded(std::string& result)
    {
        return string(&result) && finish();
    }
...
private:
    ...
    bool value(unsigned int depth)
    {
        ...
        if (cursor_ == text_.size() || depth > 32U)
        {
            return false;
        }
...
bool read_live_replay(const std::string& path, LiveReplay& replay, std::string& error)
{
    ...
    if (!frames.empty())
    {
        ...
        if (!integer(frames, parsed.frames))
        {
            return failed(error, "invalid live_frames");
        }
    }
    ...
    for (const auto control : controls)
    {
        ...
        if (!validator.reset(parsed.seed, parsed.reference_rms))
        {
            return failed(error, "live reference RMS is outside the supported range");
        }
    ...
    std::stable_sort(parsed.events.begin(), parsed.events.end(), [](const auto & left, const auto & right)
    {
