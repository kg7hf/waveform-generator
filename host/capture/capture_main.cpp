...
struct CaptureDevice
{
    ...
    bool start() noexcept { return raw ? wasapi.start() : winmm.start(); }
    void stop() noexcept { if (raw) wasapi.stop(); else winmm.stop(); }
    void close() noexcept { if (raw) wasapi.close(); else winmm.close(); }
    [[nodiscard]] bool failed() const noexcept { return raw ? wasapi.failed() : winmm.failed(); }
    [[nodiscard]] const char* error() const noexcept { return raw ? wasapi.error() : winmm.error(); }
    [[nodiscard]] std::uint32_t device_index() const noexcept { return raw ? 0U : winmm.device_index(); }
    [[nodiscard]] std::string_view native_name() const noexcept { return raw ? wasapi.native_name() : winmm.native_name(); }
    [[nodiscard]] std::string_view endpoint_id() const noexcept { return raw ? wasapi.endpoint_id() : winmm.endpoint_id(); }
    [[nodiscard]] std::string_view input_format() const noexcept { return raw ? wasapi.format_name() : WinmmCapture::input_format(); }
    [[nodiscard]] WinmmCounters counters() const noexcept { return raw ? wasapi.counters() : winmm.counters(); }
};
...
const char* reason_name(CompletionReason reason) noexcept
{
    switch (reason)
    {
    case CompletionReason::stop: return "STOP";
    case CompletionReason::host_disconnected: return "HOST_DISCONNECTED";
    case CompletionReason::duration_limit: return "DURATION_LIMIT";
    case CompletionReason::capture_error: return "CAPTURE_ERROR";
    }
...
void print_json_string(std::string_view value)
{
    ...
    for (const auto character : value)
    {
        ...
        case '\t': std::fputs("\\t", stdout); break;
        default: std::putchar(static_cast<unsigned char>(character)); break;
        }
    }
    std::putchar('"');
}
...
void print_endpoint(const CaptureDevice& capture)
{
    ...
    if (capture.endpoint_id().empty()) std::fputs("null", stdout); else print_json_string(capture.endpoint_id());
...
int main(int argc, char** argv)
{
    ...
    for (int index = 1; index < argc; ++index)
    {
        ...
        if (argument == "--list-devices") list_devices = true;
        else if (argument == "--json") json = true;
        else if (argument == "--stop-stdin") stop_stdin = true;
        ...
        else if (argument == "--device-name" && next(value)) { device_name = std::string{value}; have_name = true; }
        else if (argument == "--device-index" && next(value)) { have_index = parse_u64(value, device_index_value); }
        else if (argument == "--channel" && next(value))
        {
            if (value == "left") channel_policy = ChannelPolicy::left;
            else if (value == "right") channel_policy = ChannelPolicy::right;
            else if (value == "average") channel_policy = ChannelPolicy::average;
            else { usage(); return 2; }
        }
        else if (argument == "--sample-rate" && next(value)) { std::uint64_t ignored{}; if (!parse_u64(value, ignored) || ignored != sample_rate_hz) { usage(); return 2; } }
        else if (argument == "--max-seconds" && next(value)) have_max_seconds = parse_u64(value, max_seconds);
        else { usage(); return 2; }
    }
    ...
    if (list_devices)
    {
        ...
        if (backend_name == "winmm") static_cast<void>(WinmmCapture::enumerate(&print_device_json, nullptr));
        else if (backend_name == "wasapi-raw") static_cast<void>(WasapiCapture::enumerate(&print_wasapi_device_json, nullptr));
        else { usage(); return 2; }
        return 0;
    }
    ...
    {
        if (writer.failed.load(std::memory_order_acquire) || capture.failed()) break;
    ...
    if (stop_stdin)
    {
        reader_thread = std::thread{
            [reader]
            {
                ...
                if (line == "STOP") reader->requested.store(true, std::memory_order_release);
            }};
    }
    ...
    if (std::isfinite(queue.measurements.peak_dbfs())) std::printf("%.9g", queue.measurements.peak_dbfs()); else std::fputs("null", stdout);
    std::fputs(",\"rms_dbfs\":", stdout);
    if (std::isfinite(queue.measurements.rms_dbfs())) std::printf("%.9g", queue.measurements.rms_dbfs()); else std::fputs("null", stdout);
    std::fputs("}}\n", stdout);
    std::fflush(stdout);
    CloseHandle(queue.data_event);
