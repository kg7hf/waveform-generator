...
std::uint32_t WinmmCapture::enumerate(DeviceVisitor visitor, void* context) noexcept
{
    ...
    for (UINT index = 0U; index < count; ++index)
    {
        ...
        if (waveInGetDevCapsA(index, &capabilities, sizeof(capabilities)) == MMSYSERR_NOERROR)
        {
            ...
            if (visitor != nullptr) visitor(context, device);
        }
    }
    return count;
}
...
{
    if (handle_ == nullptr || stopped_.exchange(true, std::memory_order_acq_rel)) return;
    accepting_.store(false, std::memory_order_release);
    resetting_.store(true, std::memory_order_release);
    if (callbacks_idle_event_ != nullptr) static_cast<void>(ResetEvent(callbacks_idle_event_));
...
void WinmmCapture::complete_buffer(void* raw_header) noexcept
{
    ...
    for (; index < platform_->headers.size(); ++index)
        if (&platform_->headers[index] == header) break;
...
{
    if (callbacks_idle_event_ != nullptr) static_cast<void>(WaitForSingleObject(callbacks_idle_event_, 5000U));
}
