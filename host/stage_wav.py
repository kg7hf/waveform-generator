...
"""Prepare one known-good PCM24 WAV for the RT1170 playback diagnostic.

This staging utility accepts the firmware PCM24 format and copies the
...
class WavError(ValueError):
    """The input cannot be replayed by the firmware."""
...
def make_wgm(facts: dict[str, object], level_percent: int) -> bytes:
    ...
    if len(encoded) > 512:
        raise WavError("WGM sidecar exceeds the 512-byte limit")
    return encoded
...
def stage_wav(
    ...
    """Validate and copy *source* to the fixed card pathname."""
...
def _parser() -> argparse.ArgumentParser:
    ...
    tone_parser = subparsers.add_parser("make-tone", help="generate the deterministic ten-second tone WAV")
