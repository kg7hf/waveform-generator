...
from signal_lab.wav_io import pcm_region_sha256, read_float, require_canonical, sha256_file, wav_info, write_pcm24  # noqa: E402
...
def generate(tx, mode, out_wav, payload_path, payload_bytes, target_seconds, master_path, created_utc=None, overwrite=False):
    ...
    # Keep the external producer bytes as evidence; promote legacy PCM16 to the
    # project's PCM24 artifact format without pretending to add source precision.
    original_producer = None
    if info["encoding"] == "pcm_s16le":
        original = out_wav.with_suffix(".producer-pcm16.wav")
        if original.exists() and not overwrite:
            raise FileExistsError("refusing to replace original producer bytes: %s" % original)
        out_wav.replace(original)
        original_producer = artifact(original)
        write_pcm24(out_wav, samples)
    ...
    sidecar = {
        ...
        "producer": {"method": "external_m110", "tool_identity": artifact(tx), "argv": argv, "report": log.strip(), "original_pcm16": original_producer},
        ...
        "level": {"active_rms_dbfs": amplitude_to_db(rms), "peak_dbfs": amplitude_to_db(float(np.max(np.abs(samples)))), "clipped_samples": int(np.count_nonzero(np.abs(samples) >= 8388607 / 8388608))},
        "impairments": [],
        "conventions": {"format": "48 kHz mono PCM24; legacy producer bytes retained separately; trailing silence supplied by the transmitter",
