...
bool filename(std::string_view text, std::string_view extension) noexcept
{
    ...
    if (dot == 0 || dot > 8 || dot == text.npos || text.substr(dot + 1) != extension)
    {
        return false;
    }
...
bool number32(std::string_view text, std::uint32_t& value) noexcept
{
    ...
    if (!uint64(text, parsed) || parsed > UINT32_MAX)
    {
        return false;
    }
...
bool parse(std::string_view text, std::uint32_t& last_sequence, Request& request) noexcept
{
    ...
    for (const unsigned char c : text) if (c < 32 || c > 126)
        {
            return false;
        }
    ...
    if (space == text.npos || !uint64(text.substr(0, space), seq) || seq == 0 || seq > UINT32_MAX)
    {
        return false;
    }
    ...
    if (text.starts_with("AT:"))
    {
        ...
        if (end == text.npos || !uint64(text.substr(3, end - 3), request.frame))
        {
            return false;
        }
    ...
    if (!request.scheduled)
    {
        ...
        for (const auto& item : simple) if (text == item.name)
            {
        ...
        if (text.starts_with("PLAY:"))
        {
            ...
            for (const char c : id) if (!((c >= '0' && c <= '9') || (c >= 'a' && c <= 'f')))
                {
                    return false;
                }
        ...
        {
            request.kind = Kind::seed;
            return uint64(text.substr(5), request.integer);
        }
        ...
        if (text.starts_with("GENERATE:"))
        {
            ...
            if (!split(text, fields, count, ':') || count != 5 || !number32(fields[1], request.rate))
            {
                return false;
            }

            if (request.rate != 75 && request.rate != 150 && request.rate != 300 && request.rate != 600 &&
                    request.rate != 1200 && request.rate != 2400 && request.rate != 4800)
            {
                return false;
            }
            ...
            if (fields[2] == "short")
            {
                request.interleave = 0;
            }
            else if (fields[2] == "long")
            {
                request.interleave = 1;
            }
            else if (fields[2] == "zero")
            {
                request.interleave = 2;
            }
            else
            {
                return false;
            ...
            if (!filename(fields[3], "BIN") || !filename(fields[4], "WAV"))
            {
                return false;
            }
    ...
    if (!request.scheduled && count == 5 && words[0] == "ENCODE")
    {
        if (words[1].empty() || words[1].size() >= sizeof(request.encoder) || words[2].empty() || words[2].size() >= sizeof(request.profile) ||
                !filename(words[3], "BIN") || !filename(words[4], "WAV"))
        {
            return false;
        }

        for (const char c : words[1]) if (!((c >= 'A' && c <= 'Z') || (c >= '0' && c <= '9') || c == '_'))
            {
                return false;
            }

        for (const char c : words[2]) if (c == '"' || c == '\\')
            {
                return false;
            }
    ...
    {
        if (!number32(words[1], request.oscillator) || request.oscillator >= cw_oscillator_capacity)
        {
            return false;
        }
    ...
    {
        if (words[0] == "CW" && words[1] == "FREQ")
        {
            request.kind = Kind::cw_frequency;
        }
        else if (words[0] == "CW" && words[1] == "CI")
        {
            request.kind = Kind::cw_ci;
        }
        else if (words[0] == "STATIC" && words[1] == "RATE")
        {
            request.kind = Kind::static_rate;
        }
        else if (words[0] == "STATIC" && words[1] == "PEAK")
        {
            request.kind = Kind::static_peak;
        }
        else
        {
            return false;
    ...
    if ((count == 7 || count == 8) && words[0] == "SWEEP")
    {
        ...
        if (words[1] == "CW")
        {
            ...
            if (count == 8)
            {
                if (!number32(words[2], request.oscillator) || request.oscillator >= cw_oscillator_capacity)
                {
                    return false;
                }
            ...
            if (words[kind] == "FREQ")
            {
                request.kind = Kind::sweep_frequency;
            }
            else if (words[kind] == "CI")
            {
                request.kind = Kind::sweep_ci;
            }
            else
            {
                return false;
        ...
        {
            request.kind = Kind::sweep_fade;
            first = 2;

            if (!number32(words[6], request.duration_ms) || request.duration_ms == 0 || request.duration_ms > 3600000)
            {
                return false;
            }
        }
