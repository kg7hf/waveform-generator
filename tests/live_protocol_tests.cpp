...
static void check(bool value)
{
    if (!value)
    {
        std::fputs("live protocol assertion failed\n", stderr);
        std::abort();
    }
}
int main()
{
    ...
    check(parse("26 ENCODE TONE 1000 INPUT.BIN OUT.WAV", seq, r) &&
          r.kind == Kind::generate_file && std::string(r.encoder) == "TONE" && std::string(r.profile) == "1000");
    check(parse("27 ENCODE CUSTOM R50:S170:F1000 INPUT.BIN OUT.WAV", seq, r) && std::string(r.profile) == "R50:S170:F1000");
    check(!parse("28 ENCODE lower 1000 INPUT.BIN OUT.WAV", seq, r));
    check(!parse("29 AT:0 ENCODE TONE 1000 INPUT.BIN OUT.WAV", seq, r));
    check(!parse(std::string("30 ENCODE TONE ") + std::string(32U, 'X') + " INPUT.BIN OUT.WAV", seq, r));
    check(!parse("31 ENCODE TONE bad\"profile INPUT.BIN OUT.WAV", seq, r));
