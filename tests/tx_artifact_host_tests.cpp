...
{
    return files.contains(std::string("2:/WG/") + name);
}
std::vector<std::uint8_t>& bytes(const char* name)
{
    return files.at(std::string("2:/WG/") + name).bytes;
}
...
void fresh()
{
    failed_write_path.clear();
    short_write_path.clear();
    failed_sync_path.clear();
    failed_read_path.clear();
    failed_rename_destination.clear();
    failed_mount = failed_mkdir = player_busy = false;
    local = true;
...
Request generation(const char* output = "GEN.WAV", const char* input = "PAY.BIN", const char* profile = "4800:zero")
{
    ...
    std::strcpy(request.name, output);
    std::strcpy(request.input_name, input);
    std::strcpy(request.profile, profile);
    return request;
}
Reply upload(const std::uint8_t* input, std::size_t count)
{
    Request request;
    request.kind = Kind::data;
    request.size = static_cast<std::uint16_t>(count);
...
void verify_artifact(const char* output, const char* manifest, const char* payload)
{
    ...
    if (!exists(output) || !exists(manifest) || !exists(payload))
    {
        return;
    }
    ...
    check(std::all_of(wav.end() - 144000, wav.end(), [](std::uint8_t value)
    {
        return value == 0U;
    }), "published WAV has exactly the required silent tail region");
    check(std::any_of(wav.begin() + 44, wav.end() - 144000, [](std::uint8_t value)
    {
        return value != 0U;
    }), "generated test waveform contains nonzero signal");
    ...
    check(metadata.find("\"schema\":\"waveform-artifact/1\"") != std::string::npos &&
          metadata.find("\"encoder\":\"TEST\"") != std::string::npos &&
          metadata.find("\"complete\":true") != std::string::npos, "manifest identifies encoder and committed schema");
    check(metadata.find("\"build_id\":\"" WFG_BUILD_ID "\"") != std::string::npos,
...
void test_upload_and_publish()
{
    ...
    std::strcpy(request.name, "UPLOAD.WAV");
    check(command(request).ok, "select upload artifact name");
    request = {};
    request.kind = Kind::mode;
    request.rate = 4800U;
    request.interleave = 2U;
    ...
    for (const auto ch : std::string("CMD:SENDBUFFER"))
    {
        parser.feed(static_cast<std::uint8_t>(ch));
    }
...
void test_existing_payload_and_rejections()
{
    ...
    finish("FROMBIN.WAV");
    verify_artifact("FROMBIN.WAV", "FROMBIN.JSON", "PAY.BIN");
    ...
    request = generation();
    std::strcpy(request.encoder, "MISSING");
...
{
    fresh();
    const std::uint8_t payload[] {0U, 0xFFU, 0x55U};
    ...
    finish("GEN.WAV");
    verify_artifact("GEN.WAV", "GEN.JSON", "PAY.BIN");
    fresh();
    put("PAY.BIN", {'a'});
...
void test_io_failures()
{
    ...
    fresh();
    put("PAY.BIN", {'a', 'b', 'c'});
    failed_write_path = "2:/WG/GEN.TMP";
    ...
    fresh();
    put("PAY.BIN", {'a', 'b', 'c'});
    failed_read_path = "2:/WG/PAY.BIN";
    (void)command(generation());
    finish("GEN.WAV");
...
{"sync", "manifest", "first_rename", "second_rename"
})
    {
        fresh();
        put("PAY.BIN", {'a', 'b', 'c'});
        ...
        if (std::strcmp(fault, "sync") == 0)
        {
            failed_sync_path = "2:/WG/GEN.TMP";
        }
        else if (std::strcmp(fault, "manifest") == 0)
        {
            short_write_path = "2:/WG/GEN.JMP";
        }
        else
        {
            failed_rename_destination = std::strcmp(fault, "first_rename") == 0 ? "2:/WG/GEN.WAV" : "2:/WG/GEN.JSON";
        }
        ...
        if (std::strcmp(fault, "second_rename") == 0)
        {
            check(rename_calls == 3U && exists("GEN.JMP"), "failed metadata publish rolls the WAV name back to TMP");
        }
    }
}
} // namespace
...
extern "C"
{
    ...
    {
        if (filesystem && failed_mount)
        {
            return FR_DISK_ERR;
        }

        mounted = filesystem != nullptr;
        return FR_OK;
    }
    FRESULT f_mkdir(const char*)
    {
        ...
        const bool existed = directory;
        directory = true;
        return existed ? FR_EXIST : FR_OK;
    }
    FRESULT f_open(FIL* file, const char* path, unsigned char mode)
    {
        ...
        file->context = &found->second;
        file->mode = mode;
        file->position = 0U;
        file->data = found->second.bytes.data();
        file->size = static_cast<FSIZE_t>(found->second.bytes.size());
        file->opened = 1;
        ++open_files;
        return FR_OK;
    }
    ...
    FRESULT f_read(FIL* file, void* data, UINT requested, UINT* actual)
    {
        *actual = 0U;
        ...
        if (!file->opened || (file->mode & FA_READ) == 0U)
        {
            return FR_DISK_ERR;
        }
        ...
        if (item.path == failed_read_path)
        {
            return FR_DISK_ERR;
        }

        *actual = std::min<UINT>(requested, file->size - file->position);
        std::memcpy(data, item.bytes.data() + file->position, *actual);
    ...
    FRESULT f_write(FIL* file, const void* data, UINT requested, UINT* actual)
    {
        *actual = 0U;
        ...
        if (!file->opened || (file->mode & FA_WRITE) == 0U)
        {
            return FR_DISK_ERR;
        }
        ...
        if (item.path == failed_write_path)
        {
            return FR_DISK_ERR;
        }
        ...
        *actual = item.path == short_write_path && requested ? requested - 1U : requested;
        ...
        file->position += *actual;
        file->size = static_cast<FSIZE_t>(item.bytes.size());
        return FR_OK;
    }
    ...
    {
        if (!file->opened || offset > file->size)
        {
            return FR_DISK_ERR;
        }
    ...
    FRESULT f_stat(const char* path, FILINFO* info)
    {
        ...
        info->fsize = static_cast<FSIZE_t>(found->second.bytes.size());
        return FR_OK;
    }
    FRESULT f_rename(const char* from, const char* to)
    {
        ...
        if (!mounted || to == failed_rename_destination)
        {
            return FR_DISK_ERR;
        }
...
int main()
{
    ...
    if (failures == 0)
    {
        std::puts("TX artifact owner tests: PASS");
        return 0;
    }
